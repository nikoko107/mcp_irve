...
Lecture **littérale** du cahier des charges (tranchée explicitement avec l'utilisateur, ne pas la reconsidérer sans une nouvelle instruction) : on bufferise le réseau **BT**, on intersecte avec le réseau **routier**, et on découpe les tronçons **routiers** partiellement inclus dans ce buffer. Un `CandidateSegment.type` est donc la nature BD TOPO du tronçon routier (route, chemin...), **pas** une distinction aérien/souterrain.

**Mise à jour (nouvelle instruction utilisateur) : `point_raccordement` est maintenant le point sur le câble BT réel, pas sur la route.** Décision initiale reconsidérée : la version précédente plaçait `point_raccordement` sur le segment routier candidat le plus proche du départ (pas sur le câble), au motif que le raccordement se fait via la voirie. L'utilisateur a explicitement demandé l'inverse : le point de raccordement doit être sur le réseau électrique, la route n'étant qu'un point de passage pour l'itinéraire piéton, pas la destination finale. `pipeline.py::selectionner_meilleur_candidat` calcule maintenant `point_raccordement` via `geo/candidates.py::point_le_plus_proche_reseau_bt` (projection du point de route — `best_candidate.point_le_plus_proche` — sur le tronçon BT le plus proche), au lieu d'utiliser directement ce point de route. Le dernier tronçon (route -> câble, `distance_dernier_troncon_m`) reste calculé de la même façon, mais sa géométrie est maintenant visible sur la carte (couche `raccordement_reseau_bt`, symétrique à `acces_voirie` pour le premier tronçon) puisqu'on dispose enfin du point d'arrivée réel.
